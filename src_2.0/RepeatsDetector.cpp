...
// Cores 
const static string COR_PRM = string("-cor");
...
void drive(map<string, string> * const param) {
	...
	if (param->count(GNM_PRM) > 0) {
		...
		if (param->count(CND_PRM) > 0) {
			trainer = new Trainer(genomeDir, order, k, s, t, param->at(CND_PRM),
					minObs);
		} else {
		...
		for (unsigned int i = 0; i < chromCount; i++) {
			...
			const vector<Chromosome *> * chromList =
			...
			for (unsigned int h = 0; h < chromList->size(); h++) {
				...
				if (param->count(SCO_PRM) > 0) {
					...
					string scoFile = param->at(SCO_PRM) + Util::fileSeparator
							+ nickName + ".scr";
					if (!canAppend) {
						# pragma omp critical
						{
				...
				if (param->count(RPT_PRM) > 0) {
					...
					scanner->printIndex(rptFile, canAppend,
							atoi(param->at(FRM_PRM).c_str()));
				}

				if (param->count(MSK_PRM) > 0) {
					string mskFile = param->at(MSK_PRM) + Util::fileSeparator
							+ nickName + ".msk";
					if (!canAppend) {
						# pragma omp critical
						{
							cout << "Printing masked sequence to: " << mskFile
								<< endl;
						}
					}
		...
		if (param->count(RPT_PRM) > 0) {
			...
			cout << "Printing locations to: " << rptFile << endl;
			scanner->printIndex(rptFile, false,
					atoi(param->at(FRM_PRM).c_str()));
		}
...
int main(int argc, char * argv[]) {
	...
	validParam->insert(map<string, string>::value_type(COR_PRM, "DUMMY"));
	...
	if (argc > 1 && argc % 2 == 1) {
		for (int i = 1; i < argc - 1; i += 2) {
			if (validParam->count(argv[i]) > 0) {
				param->insert(
						map<string, string>::value_type(argv[i], argv[i + 1]));
			} else {
		...
		if(param->count(COR_PRM) > 0) {
			int corNum = atoi(param->at(COR_PRM).c_str());
			if( corNum < Util::CORE_NUM) {
				Util::CORE_NUM = corNum;
				cout << "Using " << Util::CORE_NUM << " threads.";
				cout << endl;
			} else {
				cout << "The number of threads provided it too high. ";
				cout << "Using the default of ";
				cout << Util::CORE_NUM << " threads." << endl; 
			}
		}
		...
		if (param->count(LEN_PRM) == 0) {
			if (param->count(GNM_PRM) > 0) {
				...
				for (unsigned int i = 0; i < fileList->size(); i++) {
					...
					ChromListMaker * maker = new ChromListMaker(
							fileList->at(i));
					const vector<Chromosome *> * chromList =
							maker->makeChromList();
					for (unsigned int h = 0; h < chromList->size(); h++) {
				...
				if (k < 12) {
					cout
							<< "Due to a statistical consideration, k is set to 12.";
					cout << endl;
					k = 12;
				}
				cout << endl;
				...
				param->insert(
						map<string, string>::value_type(LEN_PRM, kString));
		...
		if (param->count(FRM_PRM) == 0) {
			...
			param->insert(
					map<string, string>::value_type(FRM_PRM,
		...
		if (param->count(GNM_PRM) > 0) {
			...
			if (param->count(ORD_PRM) == 0) {
				...
				param->insert(
						map<string, string>::value_type(ORD_PRM, oString));
			}

			if (param->count(THR_PRM) == 0) {
				cout << "Using the default threshold: 2." << endl;
				param->insert(
						map<string, string>::value_type(THR_PRM, string("2")));
			} else {
			...
			if (param->count(MIN_PRM) == 0) {
				cout
						...
						<< endl;
				param->insert(
						map<string, string>::value_type(MIN_PRM, string("3")));
			} else {
				if (atoi(param->at(MIN_PRM).c_str()) < 0) {
					cerr
							<< "The minimum of the observed count of k-mers cannot be less than 0.";
					cerr << endl;
					cerr << message << endl;
					return 1;
				}
			...
			if (param->count(GAU_PRM) == 0) {
				...
				# pragma omp parallel for num_threads(Util::CORE_NUM) schedule(dynamic) reduction(+: genomeGc, genomeLength)
				for (unsigned int i = 0; i < fileList->size(); i++) {
					# pragma omp critical 
					{
						cout << "\t" << fileList->at(i) << endl;
				...
				param->insert(
						map<string, string>::value_type(GAU_PRM, wString));
			}
		} else if (param->count(HMI_PRM) > 0) {
		...
		if (param->count(DIR_PRM) > 0 && param->count(GNM_PRM) == 0) {
			cerr
					<< "Processing additional sequences is optional with -gnm only.";
			cerr << endl;
			cerr << message << endl;
			return 1;
